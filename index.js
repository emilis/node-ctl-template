/*
    Copyright 2009,2010,2012 Emilis Dambauskas

    This file is part of Cheap Tricks Library for NodeJS.
...
exports.fileToFunction = function(tpl_file_name) {
    ...
    if (!path.existsSync(tpl_file_name)) {
        throw Error(module.id + ': template file "' + tpl_file_name + '" not found.');
    }

    // Parse file contents:
    try {
        var f = this.codeToFunction( fs.readFileSync(tpl_file_name).toString() );
    } catch (error) {
...
exports.codeToFunction = function(str) {
    ...
    var code = prefix + this.compileCode(str) + suffix;
    return new Function('vars, __output_name', code);
}
